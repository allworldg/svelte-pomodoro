...
onmessage = (e) => {
  let data = e.data;
  timer = new Timer(
    parseInt(data.tomatoes),
    parseInt(data.rests),
    parseInt(data.cycles)
  );
  // isInit = false;
  timer.start();
};
...
class Timer {
  ...
  main_process() {
    ...
    if (remain_time <= 10000) {
      ...
      if (isFirstAudio) {
        if (
          this.status == RUNNING_STATUS.TOMATO ||
          (this.status == RUNNING_STATUS.REST && this.rests > 0)
        ) {
  ...
  notification() {
    if (this.status == RUNNING_STATUS.TOMATO) {
      if (this.rests > 0) {
        self.postMessage({
          notification: NOTIFICATION_TYPE.ready_rest,
          status: NOTIFICATION,
        });
      }
    } else {
      ...
      if (this.cycles == 0) {
        self.postMessage({
          notification: NOTIFICATION_TYPE.end,
          status: NOTIFICATION,
        });
      } else {
        self.postMessage({
          notification: NOTIFICATION_TYPE.ready_tomato,
          status: NOTIFICATION,
        });
      }
    }
  ...
  sendTime(remain_seconds) {
    postMessage({
      ...
      status: RUNNING,
      running_status: this.status,
      curCycles: this.curCycles,
    });
  }
}
